...
\begin{itemize}
    ...
    \item Restricciones duras ($W_v = 100000$): He asignado un peso desproporcionadamente alto (100000) a cada violación de restricción dura (solapamiento o exceso de aforo). De esta forma, cualquier solución factible siempre será matemáticamente superior a una infactible.
\end{itemize}
...
Esta estrategia me genera un punto de partida de muy baja calidad (con numerosos conflictos y violaciones), pero tiene la gran ventaja de ser computacionalmente instantánea (0.01 segundos), pasando todo el esfuerzo de optimización a la metaheurística que se encarga del resto del proceso.
...
Finalmente, la función principal \texttt{evaluar\_detallado()} las llama a todas y suma los puntos multiplicando por los pesos que definí en la teoría. Aquí es donde puse el multiplicador bestial de $100000 \cdot v$ a las violaciones para que el algoritmo huya despavorido de los horarios ilegales.
...
\begin{itemize}
    \item \textbf{Salida por consola:} Imprime un registro detallado de las evaluaciones, penalizaciones y violaciones de cada algoritmo paso a paso, culminando en la impresión de una tabla resumen perfectamente tabulada con los costes y tiempos comparados.
...
Aunque no se haya llegado a 0 violaciones debido a la altísima densidad de la instancia (100 exámenes y 2000 estudiantes para solo 10 aulas) y al número máximo de evaluaciones impuesto, se puede observar una reducción drástica respecto al inicio, demostrando que la heurística funciona.

\begin{figure}[H]
    \centering
    % Nota: Crea o adjunta una gráfica de violaciones, si no la tienes, puedes comentar esta figura.
    \includegraphics[width=0.7\textwidth]{graficas/comparativa_costes.png} 
    \caption{Evolución del número de violaciones/coste de restricciones duras según el algoritmo utilizado.}
\end{figure}
...
Lo verdaderamente destacable es que la estrategia de Primer Mejor ha dado un resultado más eficiente con una diferencia abismal con respecto al de la estrategia del Mejor Vecino, obteniendo un coste muchísimo menor (8.7 millones frente a 22.7 millones) y reduciendo las violaciones a casi un tercio, invirtiendo además un poco menos de tiempo.
